...
fn create(
    N: usize,
    M: usize,
    scan: &mut Scanner<io::Stdin>,
) -> Result<HashMap<usize, Vec<usize>>, StopCode> {
    let mut G: HashMap<usize, Vec<usize>> = HashMap::with_capacity(N);
    for _ in 0..M {
        let (u, v) = scan.take_tuple::<Uint, Uint>()?;
        if let Some(neighbours) = G.get_mut(&u) {
            neighbours.push(v);
        } else {
            G.insert(u, vec![v]);
        }
...
#[allow(non_snake_case)]
fn traverse(G: HashMap<usize, Vec<usize>>, S: &mut HashSet<usize>) {
    let mut stack = vec![(1usize, 0)];
    while let Some((u, d)) = stack.pop() {
        let l1 = stack.len();
        if let Some(neighbours) = G.get(&u) {
            neighbours.iter().fold((), |_, &v| stack.push((v, d + 1)));
        }
        let l2 = stack.len();
        if l2 == l1 {
            S.insert(d);
...
fn main() -> Result<(), StopCode> {
    ...
    let (N1, N2) = scan.take_tuple::<Uint, Uint>()?;
    let (M1, M2) = scan.take_tuple::<Uint, Uint>()?;
    let (G1, G2) = (create(N1, M1, &mut scan)?, create(N2, M2, &mut scan)?);
    ...
    for _ in 0..Q {
        let q = scan.next::<Uint>()?;
        writeln!(out, "{}", if S.contains(&q) { "Yes" } else { "No" })?;
    }
    Ok(out.flush()?)
}
