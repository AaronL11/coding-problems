...
fn solve(
    ...
    let (Q,M) = scan.take_tuple::<u32,u64>()?;
    let (B,F) = scan.take_tuple::<u64,u64>()?;
    let mut clouds: HashMap<u64,u64> = HashMap::new();
    let (mut sum, mut d) = (0,0);
    for _ in 0..Q {
        match scan.next::<u64>()? {
            1 => {
                let (P,H) = scan.take_tuple::<u64,u64>()?;
                clouds.insert(P,H);
            }
            2 => {
                let X = scan.next::<u64>()?;
        ...
        let mut hit = 0;
        while d<M {
            let h = match clouds.get(&d) {
                Some(v) => *v,
                None => 0
            };
            d += if hit%2==0 { F } else { B } + h;
            sum += 1;
            hit += 1;
        }
        writeln!(out,"{}",sum)?;
