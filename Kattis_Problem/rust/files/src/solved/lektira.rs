...
fn split_sort(a: &[u8], i: usize, j: usize) -> Vec<u8> {
    let n = a.len();
    let (mut l, mut m, mut r) = (vec![0; i], vec![0; j - i], vec![0; n - j]);
    l.copy_from_slice(&a[..i]);
    m.copy_from_slice(&a[i..j]);
    r.copy_from_slice(&a[j..]);
    l.iter()
        .rev()
        .chain(m.iter().rev())
        .chain(r.iter().rev())
...
fn main() -> Result<(), StopCode> {
    ...
    let s = scan.get_str()?.as_bytes().to_owned();
    let n = s.len();
    let mut a = vec![];
    for i in 1..n - 1 {
        for j in i + 1..n {
            a.push(split_sort(&s[..], i, j));
        }
    }
    a.sort();
    write!(
        out,
        "{}",
        a[0].iter().map(|r| *r as char).collect::<String>()
    )?;
    Ok(out.flush()?)
}
