...
fn solve(
    ...
    let mut toys = HashMap::with_capacity(100);
    for _ in 0..scan.next::<u8>()? {
        for _ in 0..scan.next::<u8>()? {
            let name = scan.get_str()?.to_owned();
            let n = scan.next::<u8>()?;
            if let Some(toy) = toys.get_mut(&name) {
                *toy += n;
            } else {
                toys.insert(name,n);
            }
        }
        let mut v = toys.iter().collect::<Vec<_>>();
        v.sort_by(|(ka,va),(kb,vb)| if va==vb { ka.cmp(&kb) } else { vb.cmp(&va) });
        // dbg!(&toys,&v);
        writeln!(out,"{}",v.len())?;
        for (k,v) in v {
            writeln!(out,"{} {}",k,v)?;
