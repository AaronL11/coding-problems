...
fn main() -> Result<(), StopCode> {
    ...
    let coins = scan.take_into::<Uint, Vec<_>>(n);
    let l = coins[coins.len() - 1] + coins[coins.len() - 2];
    let mut greedy = vec![0; l];
    let mut j = 0;
    for i in 1..l {
        if j + 1 < n && i == coins[j + 1] {
            j += 1;
        }
        greedy[i] = 1 + greedy[i - coins[j]];
    }
    ...
    for i in 1..l {
        dp[i] = 1 + coins
            .iter()
            .filter_map(|&c| if c <= i { Some(dp[i - c]) } else { None })
            .min()
            .unwrap();
    }
    if greedy == dp {
