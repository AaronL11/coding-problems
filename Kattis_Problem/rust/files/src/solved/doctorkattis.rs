...
fn main() -> Result<(), StopCode> {
    ...
    let mut catsh = HashMap::new();
    let n = scan.next::<i64>()?;
    for j in 0..n {
        let t = scan.next::<Uint>()?;
        match t {
            0 => {
                let s = scan.get_str()?.to_owned();
                let s1 = s.clone();
                let i = scan.next::<Uint>()?;
                cats.insert((i, -j), s);
                catsh.insert(s1, (i, -j));
            }
            1 => {
                ...
                let s1 = s.clone();
                let i = scan.next::<Uint>()?;
                let (x, k) = catsh.get_mut(&s).unwrap();
                cats.remove(&(*x, *k)).unwrap();
                *x += i;
                cats.insert((*x, *k), s1);
            }
            2 => {
                let s = scan.get_str()?.to_owned();
                let i = catsh.remove(&s).unwrap();
                cats.remove(&i);
            }
            3 => {
                if let Some(n) = cats.values().last() {
                    writeln!(out, "{}", n)?;
                } else {
