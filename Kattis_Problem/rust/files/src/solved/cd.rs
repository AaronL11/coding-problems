...
fn solve(
    ...
    loop {
        ...
        let jk = scan.take::<u64>(N)?;
        let jl = scan.take::<u64>(M)?;
        let Jk: HashSet<&u64> = jk.iter().collect();
        let Jl: HashSet<&u64> = jl.iter().collect();
        let un: HashSet<&&u64> = Jk.intersection(&Jl).collect();
        writeln!(out,"{}",un.len())?;
    }
    Ok(out.flush()?)
}
