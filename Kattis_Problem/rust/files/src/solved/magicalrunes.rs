...
fn main() -> Result<(), StopCode> {
    ...
    let (rune, m) = scan.get_str()?.bytes().fold((0, 0), |(acc, i), b| {
        (acc + if b == b'A' { 0 << i } else { 1 << i }, i + 1)
    });
    let days = scan.next::<Uint>()?;
    let mut n = rune + days;
    let mut runes = vec![];
    for _ in 0..m {
        runes.push(if (n & 1) == 1 { b'B' } else { b'A' });
        n >>= 1;
    }
    writeln!(out, "{}", unsafe { str::from_utf8_unchecked(&runes[..]) })?;
    Ok(out.flush()?)
}
