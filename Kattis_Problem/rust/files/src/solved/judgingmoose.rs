...
fn solve(
    ...
    let (l,r) = scan.take_tuple::<u8,u8>()?;
    if (l,r)==(0,0) {
        writeln!(out,"Not a moose")?;
    } else {
        match l.cmp(&r) {
            Ordering::Less => writeln!(out,"Odd {}", r*2)?,
            Ordering::Equal => writeln!(out,"Even {}", r+l)?,
            Ordering::Greater => writeln!(out,"Odd {}", l*2)?
        }
    }
    Ok(out.flush()?)
}
