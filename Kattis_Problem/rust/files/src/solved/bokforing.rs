...
fn solve(
    ...
    let (_,Q) = scan.take_tuple::<u32,u32>()?;
    let mut people: HashMap<u32,u32> = HashMap::new();
    let mut set = 0;
    for _ in 0..Q {
        let command = scan.get_str()?.clone();
        match command {
            "SET" => {
                let (i,x) = scan.take_tuple::<u32,u32>()?;
                people.insert(i, x);
            }
            "RESTART" => {
                set = scan.next::<u32>()?;
            ...
            "PRINT" => writeln!(
                out,
                "{}",
                people.get(&scan.next::<u32>()?).or(Some(&set)).unwrap()
                )?,
            _ => panic!()
        }
    }
