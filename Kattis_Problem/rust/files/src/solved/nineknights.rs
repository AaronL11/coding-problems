...
fn main() -> Result<(), StopCode> {
    ...
    let board = (0..5)
        .flat_map(|_| scan.next::<String>())
        .map(|s| s.chars().collect::<Vec<_>>())
        .collect::<Vec<_>>();
    let mut valid = true;
    let changes: [(isize, isize); 8] = [
        (-2, -1),
        (-1, -2),
        (2, 1),
        (1, 2),
        ...
        for j in 0..5 {
            if board[i][j] == 'k' {
                ...
                for (dr, dc) in changes {
                    let di = i as isize + dr;
                    let dj = j as isize + dc;
                    if di >= 0 && dj >= 0 {
                        if let Some(row) = board.get(di as usize) {
                            if let Some(piece) = row.get(dj as usize) {
                                if *piece == 'k' {
                                    valid = false;
                                    break 'main;
                                }
