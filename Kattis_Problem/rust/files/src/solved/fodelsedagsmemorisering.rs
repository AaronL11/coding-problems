...
fn solve(
    ...
    let n = scan.next::<u64>()?;
    let mut bdays: HashMap<String,Vec<(u64,String)>> = HashMap::new();
    for _ in 0..n {
        let (n,l,b) = scan.take_tuple3::<String,u64,String>()?;
        if bdays.contains_key(&b) {
            bdays.get_mut(&b).unwrap().push((l,n));
        } else {
            bdays.insert(b.to_string(),vec![(l,n)]);
        }
    }
    let mut v: Vec<String> = bdays.values()
                                .map(|v| 
                                    if v.len()>1 {
                                        v.iter()
                                        .fold(
                                            (0,String::from("")),|(max,y),(i,x)|
                                            if max <= *i {
                                                (*i,x.to_string())
                                            } else {
                                                (max,y)
