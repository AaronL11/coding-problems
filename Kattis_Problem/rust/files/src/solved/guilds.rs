...
fn main() -> Result<(), StopCode> {
    ...
    for i in 0..n {
        let (a, b) = scan.take_tuple::<String, String>()?;
        map.insert(a.to_owned(), i);
        rev.insert(i, a.to_owned());
        if !map.contains_key(&b) {
            res.insert(b.to_owned());
        }
        seq.push((a, b));
    }
    for (i, g) in res.iter().enumerate() {
        if !map.contains_key(g) {
            map.insert(g.to_owned(), n + i);
            rev.insert(n + i, g.to_owned());
        }
    }
    for (a, b) in seq {
        uf.union(map[&b], map[&a]);
        guilds.push(a);
    }
    for g in guilds {
        writeln!(out, "{} {}", g, rev[&uf.find(map[&g])])?;
    }
    Ok(out.flush()?)
}
