...
fn main() -> Result<(), StopCode> {
    ...
    let msg = scan
        ...
        .collect::<Vec<_>>();
    let n = msg.len();
    let (h1, h2) = (&msg[..n / 2], &msg[n / 2..]);
    let r1 = h1.iter().sum::<usize>();
    let r2 = h2.iter().sum::<usize>();
    let h1 = h1.iter().map(|b| (*b + r1) % 26).collect::<Vec<_>>();
    let h2 = h2.iter().map(|b| (*b + r2) % 26).collect::<Vec<_>>();
    let res = (0..n / 2)
        .map(|i| ((h1[i] + h2[i]) % 26) as u8 + 65)
        .collect::<Vec<u8>>();
    unsafe {
        writeln!(out, "{}", str::from_utf8_unchecked(&res))?;
    }
    Ok(out.flush()?)
}
