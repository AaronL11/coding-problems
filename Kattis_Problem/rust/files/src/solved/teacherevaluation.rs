...
fn main() -> Result<(), StopCode> {
    ...
    let (N, P) = scan.take_tuple::<Uint, f64>()?;
    let scores = scan.take_into::<Uint, Vec<_>>(N);
    let mut sum = scores.iter().sum::<Uint>() as f64;
    let mut n = scores.len() as f64;
    let mut avg = sum / n;
    let mut cnt = 0;
    if (avg - P).abs() < EPSILON {
        writeln!(out, "0")?;
    } else if P == 100. {
