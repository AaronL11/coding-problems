...
type UF = [Uint; 1_000_001];

fn union(g: &mut UF, a: Uint, b: Uint) {
    let (la, lb) = (find(g, a), find(g, b));
    let min = std::cmp::min(la, lb);
    g[la] = min;
    g[lb] = min;
}

fn find(g: &mut UF, a: Uint) -> Uint {
    let mut i = a;
    while g[i] != i {
        g[i] = g[g[i]];
        i = g[i]
...
where
    R: Read,
{
    let mut groups = [0; 1_000_001];
    for (i, x) in groups.iter_mut().enumerate() {
        *x = i;
    }
    let (_, Q) = scan.take_tuple::<Uint, Uint>()?;
    for _ in 0..Q {
        let c = scan.get_str()?.bytes().next();
        let (a, b) = scan.take_tuple::<Uint, Uint>()?;
        match c {
            Some(b'=') => union(&mut groups, a, b),
            Some(b'?') => writeln!(
                out,
                "{}",
                match find(&mut groups, a) == find(&mut groups, b) {
                    true => "yes",
                    false => "no",
                }
            )?,
