...
fn main() -> Result<(), StopCode> {
    ...
    let mut last = scan.next::<String>()?;
    let mut set = HashSet::from([last.clone()]);
    let mut b = true;
    for i in 1..n {
        let word = scan.next::<String>()?;
        if set.contains(&word) || word[0..1] != last[last.len() - 1..] {
            writeln!(out, "Player {} lost", i % 2 + 1)?;
            b = false;
            break;
        }
        last = word.clone();
