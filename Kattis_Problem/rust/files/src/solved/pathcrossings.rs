...
fn main() -> Result<(), StopCode> {
    ...
    let mut C = vec![];
    let (_, N) = scan.take_tuple::<Uint, Uint>()?;
    let mut v = vec![];
    for _ in 0..N {
        let (p, x) = scan.take_tuple::<Int, Int>()?;
        let (y, t) = scan.take_tuple::<Int, Int>()?;
        v.push((t, x, y, p));
    }
    v.sort();
    for i in 0..v.len() {
        ...
        while j <= 5 && i + j < v.len() {
            let (u, w, z, q) = v[i + j];
            let d = (((x - w) * (x - w) + (z - y) * (z - y)) as f64).sqrt();
            if (u - t).abs() <= 10 && d <= 1000. && p != q {
                if p < q {
                    C.push((p, q));
                } else {
                    C.push((q, p))
