...
fn main() -> Result<(), StopCode> {
    ...
    let keys = scan.get_str()?.as_bytes().to_owned();
    let mut words = vec![];
    ...
    let mut res = true;
    if keys.len() != words.len() {
        res = false;
    } else {
        for word in 0..words.len() {
            if let Some(w) = map.get(&keys[word]) {
                if *w != words[word] {
                    res = false;
                    break;
                }
            }
            if let Some(&w) = map2.get(&words[word]) {
                if w != keys[word] {
                    res = false;
                    break;
                }
            }
            map.insert(keys[word], words[word].clone());
            map2.insert(words[word].clone(), keys[word]);
        }
    }
    if res {
        writeln!(out, "True")?;
