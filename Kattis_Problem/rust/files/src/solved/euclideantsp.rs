...
fn calc(c: f64, n: f64, p: f64, s: f64, v: f64) -> f64 {
    (n*(n.log2().powf(c*2f64.sqrt())))/(p*1e9) + s*(1f64+c.recip())/v
}
...
fn solve(
    ...
    let (n,p) = scan.take_tuple::<f64,f64>()?;
    let (s,v) = scan.take_tuple::<f64,f64>()?;
    let (mut hi, mut low) = (1e2f64,1e-9f64);
    let mut third;
    let (mut c1,mut c2);
    while (hi-low).abs() > 1e-6 {
        third = (hi-low)/3.;
        c1 = low + third;
        c2 = hi - third;
        if calc(c1,n,p,s,v) > calc(c2,n,p,s,v) {
            low = c1;
        } else {
            hi = c2;
        }
    }
    writeln!(out,"{} {}",calc(hi,n,p,s,v),hi)?;
    Ok(out.flush()?)
}
