...
fn main() -> Result<(), StopCode> {
    ...
    let mut days = [0; 51];
    let (c, n, m) = scan.take_tuple3::<Uint, Uint, Uint>()?;
    let mut map = (1..=c).map(|i| (i, 0)).collect::<HashMap<Uint, Uint>>();
    days[0] = (0..n)
        .flat_map(|_| scan.next::<Uint>())
        .map(|i| {
            if let Some(num) = map.get_mut(&i) {
                *num += 1;
            }
            i
        })
    ...
    for i in 1..=50 {
        for j in (1..=c).rev() {
            ...
            if k > c {
                let w = k / 2;
                if let Some(num) = map.get_mut(&w) {
                    *num += f;
                }
            } else {
                if let Some(num) = map.get_mut(&k) {
                    *num += f;
                }
                if let Some(num) = map.get_mut(&j) {
                    *num = 0;
                }
            }
        }
        days[i] = map.values().sum::<Uint>();
    }
    for _ in 0..m {
        let day = scan.next::<Uint>()?;
        writeln!(out, "{}", days[day])?;
    }
    Ok(out.flush()?)
}
