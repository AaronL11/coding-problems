...
fn main() -> Result<(), StopCode> {
    ...
    for _ in 0..scan.next::<Uint>()? {
        let (x1, y1, z1) = scan.take_tuple3::<f64, f64, f64>()?;
        let r1 = scan.next::<f64>()?;
        let (vx1, vy1, vz1) = scan.take_tuple3::<f64, f64, f64>()?;
        let (x2, y2, z2) = scan.take_tuple3::<f64, f64, f64>()?;
        let r2 = scan.next::<f64>()?;
        let (vx2, vy2, vz2) = scan.take_tuple3::<f64, f64, f64>()?;
        if (vx1, vy1, vz1) == (0., 0., 0.) && (vx2, vy2, vz2) == (0., 0., 0.) {
            writeln!(out, "No collision")?;
        } else {
            let (dx, dy, dz) = (x1 - x2, y1 - y2, z1 - z2);
            let (dvx, dvy, dvz) = (vx1 - vx2, vy1 - vy2, vz1 - vz2);
            let r = r1 + r2;
            let dr2 = dx * dx + dy * dy + dz * dz;
            let dvdr = dvx * dx + dvy * dy + dvz * dz;
            let dv2 = dvx * dvx + dvy * dvy + dvz * dvz;
            let (a, b, c) = (dv2, 2. * dvdr, dr2 - r * r);
            let det = b * b - 4. * a * c;
            if b >= 0. {
                writeln!(out, "No collision")?;
            } else if det == 0. {
                ...
                let (t1, t2) = ((-b - det.sqrt()) / (2. * a), (-b + det.sqrt()) / (2. * a));
                if t1 < 0. && t2 < 0. {
                    writeln!(out, "No collision")?;
                } else if t1 < 0. {
                    writeln!(out, "{:.3}", t2)?;
                    ...
                    writeln!(out, "{:.3}", if t1 <= t2 { t1 } else { t2 })?;
                }
            }
        }
    }
