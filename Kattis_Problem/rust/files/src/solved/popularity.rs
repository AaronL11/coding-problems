...
let (N,M) = scan.take_tuple::<i64,i64>()?;
    let mut succ = (1..=N as i64).map(|i|-i).collect::<Vec<i64>>();
    for _ in 0..M {
...
fn solve(
    ...
    let n = scan.next::<u32>()?;
    let (mut pe,mut sum) = (0,0);
    for _ in 0..n {
        let (s,f) = scan.take_tuple::<u32,u32>()?;
        if s>=pe {
            sum += 1;
            pe = f;
        }
