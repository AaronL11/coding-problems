...
fn main() -> Result<(), StopCode> {
    ...
    let (N, M) = scan.take_tuple::<String, String>()?;
    if &M[..] == "1" {
        writeln!(out, "{}", N)?;
    } else {
        let mut f = N.chars().rev().take_while(|&c| c == '0').count();
        let ans: String;
        let (n, m) = (N.len(), M.len());
        if n < m {
            ans = iter::once('0')
                .chain(iter::once('.'))
                .chain((0..m - 1 - n).map(|_| '0'))
                .chain(N[..n - f].chars())
                .collect::<String>();
        } else if n == m {
            ans = iter::once(N.as_bytes()[0] as char)
                .chain(iter::once('.').chain(N[1..f].chars()))
                .collect::<String>();
        } else {
            if f >= m - 1 {
                f = m - 1;
                ans = N[..n - f].chars().collect::<String>();
            } else {
                ans = N[..n - m + 1]
                    .chars()
                    .chain(iter::once('.').chain(N[n - m + 1..n - f].chars()))
                    .collect::<String>();
            }
        }
        writeln!(out, "{}", ans)?;
