...
fn main() -> Result<(), StopCode> {
    ...
    while let Ok(n) = scan.next::<Int>() {
        ...
        let divsum = divs.iter().sum::<Int>() - n;
        if divsum == n {
            writeln!(out, "{} perfect", n)?;
        } else if (n - divsum).abs() <= 2 {
            writeln!(out, "{} almost perfect", n)?;
        } else {
            writeln!(out, "{} not perfect", n)?;
        }
    }
    Ok(out.flush()?)
}
