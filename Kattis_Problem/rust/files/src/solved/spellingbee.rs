...
fn solve(
    ...
    let chars = scan.get_str()?.to_owned();
    let charset: HashSet<char> = HashSet::from_iter(chars.chars());
    ...
    for _ in 0..n {
        ...
        if s.len() < 4 {
            continue
        } else if s.contains(&chars[0..1]) {
            let sset: HashSet<char> = HashSet::from_iter(s.chars());
            let temp: HashSet<&char> = HashSet::from_iter(sset.difference(&charset));
            if temp.is_empty() {
                words.push(s);
            }
        }
